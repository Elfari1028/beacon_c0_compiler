...
public class Assembler {
    ...
    public void output() throws IOException, CompileError {
        ...
        // for (int i = 0; i < top; i++) {
        // System.out.println(
        // globals[i].getIsConst() + " " + globals[i].getValueCount() + " " +
        ...
        for (int i = globalVarsEnd + 8; i < globalCount; i++) {
            ...
            int loc_slots = funcEntry.getLocalVariables().size();
            ...
            if (funcName.equals("_start")) {
                ...
                functions[funcTableTop++] = new Function(i, ret_slots, param_slots, loc_slots, body_count,
                        instructionEntries);
            }
        }
        ...
        // System.out.println(
        // functions[i].getNameLoc() + " " + functions[i].getRetSlots() + " " +
        // functions[i].getParamSlots()
        // + " " + functions[i].getLocSlots() + " " + functions[i].getBodyCount());
        // for (int j = 0; j < functions[i].getBodyCount(); j++) {
        // System.out.println(functions[i].getInstructions().get(j).getIns() + "("
        // + functions[i].getInstructions().get(j).getOp() + ")");
        // }
        // }
        ...
        for (int i = 0; i < functionCount; i++) {
            ...
            output.addAll(name);
            // retSlots
            List<Byte> retSlots = int2bytes(4, functions[i].getRetSlots());
            output.addAll(retSlots);
            // paramsSlots;
            List<Byte> paramsSlots = int2bytes(4, functions[i].getParamSlots());
            output.addAll(paramsSlots);
            // locSlots;
            List<Byte> locSlots = int2bytes(4, functions[i].getLocSlots());
            output.addAll(locSlots);
            // bodyCount
            List<Byte> bodyCount = int2bytes(4, functions[i].getBodyCount());
